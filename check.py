...
#     BACKEND_URL = "http://127.0.0.1:5000/transcript"
...
def apps():
    ...
    def fetch_transcript(video_id):
        try:
            transcripts = yti.list(video_id=video_id)
            manual_transcripts = [t for t in transcripts if not t.is_generated]
            if manual_transcripts:
                transcript = manual_transcripts[0]
            else:
            ...
            return yti.fetch(video_id=video_id, languages=[language_code])
        except NoTranscriptFound:
